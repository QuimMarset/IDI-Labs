...
void MyGLWidget::enviaPosFocus() {
  glm::vec3 posFocusSCO;

  if (tipusFocus) {
    posFocus = glm::vec3(posX, -0.5, posZ);  // en SCA
...
void MyGLWidget::enviaParametresIlum() {
  ...
  glm::vec3 colFocus = glm::vec3(0.8, 0.8, 0.0);
  glm::vec3 llumAmbient = glm::vec3(0.2, 0.2, 0.0);
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG,glm::vec3(posX,-1.0,posZ));
...
void MyGLWidget::calculaCapsaModel ()
{
  ...
  centreBaseCapsaPatr = glm::vec3((maxx + minx)/2.0, miny, (maxz + minz)/2.0);
}
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    /*case Qt::Key_K: {
      posFocusX -= 0.1f;
      glm::vec3 posFocusAux = posFocus;
      posFocusAux.x = posFocusX;
      posFocusAux = glm::vec3(View*glm::vec4(posFocusAux,1.0));
      glUniform3fv(posFocusLoc,1,&posFocusAux[0]);
      break;
    }
    case Qt::Key_L: {
