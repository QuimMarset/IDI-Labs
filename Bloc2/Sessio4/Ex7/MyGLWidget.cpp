...
void MyGLWidget::mouseMoveEvent(QMouseEvent* event) {
  ...
  viewTransform();
  int angleEnviarX,angleEnviarY;
  angleEnviarX = angleX * 180.0/M_PI;
  angleEnviarY = angleY* 180.0/M_PI;
  if (angleEnviarY >= 360) angleEnviarY = angleEnviarY%360;
  if (angleEnviarX >= 360) angleEnviarX = angleEnviarX%360;
  emit enviaAngleX(angleEnviarX);
  emit enviaAngleY(angleEnviarY);
...
void MyGLWidget::carregaBuffersColorTerra() {
...
void MyGLWidget::repCameraPersp(bool select) {
  makeCurrent();
  if (select) cameraSeleccionada = true;
  else cameraSeleccionada = false;
  projectTransform();
  update();
}

void MyGLWidget::repCameraOrt(bool select) {
  makeCurrent();
  if (select) cameraSeleccionada = false;
  projectTransform();
  update();
}

void MyGLWidget::repAngleX(int angle) {
  makeCurrent();
  angleX = angle * M_PI/180.0;
  viewTransform();
  update();
}

void MyGLWidget::repAngleY(int angle) {
  makeCurrent();
  angleY = angle * M_PI/180.0;
  viewTransform();
  update();
}

void MyGLWidget::repColorR(int value) {
  makeCurrent();
  colorR = (float) value/255.0;
  carregaBuffersColorTerra();
  update();
}

void MyGLWidget::repColorG(int value) {
  makeCurrent();
  colorG = (float)value/255.0;
  carregaBuffersColorTerra();
  update();
}

void MyGLWidget::repColorB(int value) {
  makeCurrent();
  colorB = (float) value/255.0;
  carregaBuffersColorTerra();
  update();
}

void MyGLWidget::repW(int value) {
  makeCurrent();
  viewportW = value;
  update();
}

void MyGLWidget::repH(int value) {
  makeCurrent();
  viewportH = value;
  update();
}