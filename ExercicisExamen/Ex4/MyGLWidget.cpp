...
void MyGLWidget::enviaParametresIlum() {
  ...
  glm::vec3 colFocus = glm::vec3(0.0, 0.8, 0.8);
...
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG,glm::vec3(0,2.0f,0));
...
void MyGLWidget::iniCamera() {
  ...
  glm::vec3 puntMaximEscena = glm::vec3(2.0,3.0,2.0);
...
void MyGLWidget::projectTransform ()
{
  ...
  if (perspectiva) {
    if (rav >= 1.0f) {
      Proj = glm::perspective(2.0f*angleZoom,rav,zN,zF);
    }
    else {
  ...
  if (e->buttons() == Qt::RightButton) {
    ...
    if (e->y() < yClick) {
      if (angleAux >= 15.0) {
        angleZoom -= 0.05f;
        projectTransform();
        int angleEnviar = angleZoom * 180/M_PI;
        emit enviaZoom(angleEnviar);
     }
    }
    else if (e->y() > yClick) {
      if (angleAux <= 60.0) {
...
void MyGLWidget::repZoom(int value) {
  makeCurrent();
  angleZoom = (float)value * M_PI/180.0;
  projectTransform();
  update();
}
