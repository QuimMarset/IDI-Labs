...
void MyGLWidget::iniCamera() {
  ...
  glm::vec3 puntMaximEscena = glm::vec3(2.0,1.5,2.0);
  ...
  if (canviCamera == false) {
    zN = radiEsc;
    angleCamera = asin(radiEsc/dist);
  }
  else { //Camera sobre el patricio
    zN = 0.1f;
    angleCamera = M_PI/4.0f;
    UP = glm::vec3(0,1,0);
...
{
  glm::mat4 Proj;  
  if (perspectiva) { //Camera perspectiva
    if (rav >= 1.0f) {
  ...
  else { //Camera ortogonal
    if (rav >= 1.0f) {
      float lAux = -radiEsc*rav;
      float rAux = radiEsc*rav;
      Proj = glm::ortho(lAux,rAux,b,t,zN,zF);
    }
    else {
      float bAux = -radiEsc/rav;
      float tAux = radiEsc/rav;
      Proj = glm::ortho(l,r,bAux,tAux,zN,zF);
    }
  }
...
void MyGLWidget::enviaPosFocus() {
  ...
  glm::vec3 posFocusAux = glm::vec3(posX,2.0,0.0);

  glm::vec3 posFocusSCO = glm::vec3(View*glm::vec4(posFocusAux,1.0f));
...
void MyGLWidget::calculaCapsaModel ()
{
  ...
  escalaPatr1 = 1.0/(maxy-miny);
  escalaPatr2 = 2.5/(maxy - miny);
  centrePatr[0] = (minx+maxx)/2.0; centrePatr[1] = (miny+maxy)/2.0; centrePatr[2] = (minz+maxz)/2.0;
  centreBaseCapsaPatr = glm::vec3 ((minx+maxx)/2.0, miny, (minz+maxz)/2.0);
}
...
void MyGLWidget::enviaComponentDifusaTerra() {

  glm::vec3 diff(diffR,diffG,diffB);
...
void MyGLWidget::repTipusCamera(int estat) {
  makeCurrent();
  if (estat == 0) {
    canviCamera = false;
  }
...
void MyGLWidget::repDiffR(int value) {
  makeCurrent();
  diffR = float(value/100.0);
  enviaComponentDifusaTerra();
  update();
}

void MyGLWidget::repDiffG(int value) {
  makeCurrent();
  diffG = float(value/100.0);
  enviaComponentDifusaTerra();
  update();
}

void MyGLWidget::repDiffB(int value) {
  makeCurrent();
  diffB = float(value/100.0);
  enviaComponentDifusaTerra();
  update();
}